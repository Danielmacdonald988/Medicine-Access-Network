...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t('Account setup issue') }
}
    ...
    <div className="flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md border-stone-200">
        <CardHeader className="space-y-1">
          ...
          <CardTitle className="text-xl">{t("We hit a snag setting up your account")}</CardTitle>
          <CardDescription>
            {t(message)}{' '}{t("Signing out and back in often resolves it — if it keeps happening, please contact support.")}</CardDescription>
        </CardHeader>
        <CardContent>
          <form action="/api/auth/signout" method="post">
            <Button type="submit" className="w-full bg-emerald-700 hover:bg-emerald-800">{t("Sign out and try again")}</Button>
          </form>
        </CardContent>
        <CardFooter className="flex justify-center border-t border-stone-100 pt-4">
          <p className="text-sm text-stone-500">{t("Still stuck?")}{' '}
            <Link href="/" className="font-medium text-emerald-700 hover:underline">{t("Return home")}</Link>{' '}{t("or contact support.")}</p>
        </CardFooter>
      </Card>
    </div>
  )
