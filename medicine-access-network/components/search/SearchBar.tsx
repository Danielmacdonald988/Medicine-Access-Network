...
export function SearchBar({ onSearch }: { onSearch?: () => void }) {
  ...
  return (
    <form
      ...
      method="get"
      role="search"
      aria-label={t("Guide directory")}
      onSubmit={handleSubmit}
      className="rounded-xl border border-stone-300 bg-white p-4 shadow-[0_10px_30px_#1e37260c] sm:p-6"
      aria-busy={isPending}
    >
      ...
      <div className="grid gap-4 md:grid-cols-[1.3fr_1fr_auto] md:items-end">
        <div>
          ...
          <div className="relative">
            ...
            <Input
              ...
              maxLength={120}
              placeholder={t("Preparation, integration, or a name")}
              className="h-12 border-stone-200 bg-stone-50/60 pl-9 text-base shadow-none"
            />
          </div>
        </div>
        <div>
        ...
        <Button
          type="submit"
          disabled={isPending}
          className="h-12 bg-emerald-700 px-7 hover:bg-emerald-800"
        >
          {t(isPending ? "Searching…" : "Find a guide")}
        </Button>
      </div>
...
export function SearchSort() {
  ...
  return (
    <div
      ...
      aria-busy={isPending}
    >
      <label htmlFor="guide-sort" className="text-sm text-stone-600">
        {t("Sort by")}
      </label>
      <select
        id="guide-sort"
        ...
        className="min-h-11 min-w-0 max-w-full rounded-lg border border-stone-300 bg-white px-3 text-sm text-stone-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700"
      >
        <option value="newest">{t("Newest profiles")}</option>
        <option value="name">{t("Name A–Z")}</option>
      </select>
      <span role="status" className="sr-only">
        {isPending ? t("Sorting guide results…") : ""}
      </span>
    </div>
  );
}
