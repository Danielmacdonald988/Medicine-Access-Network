...
function Comparison({ ids, onRemove }: { ids: string[]; onRemove: (id: string) => void }) {
  ...
  if (state.status === 'loading') return <p role="status" className="rounded-2xl border border-stone-200 bg-white p-8 text-stone-600">Checking current public profiles…</p>
  ...
  return <div>
    ...
    <div role="region" aria-label="Guide comparison; scroll horizontally on small screens" tabIndex={0} className="overflow-x-auto rounded-2xl border border-stone-200 bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700">
      <table className="w-full table-fixed text-left text-sm" style={{ minWidth: 180 + ids.length * 230 }}>
        ...
        <tbody>
          {rows.map(({ label, value }) => <tr key={label} className="border-b border-stone-100 last:border-0">
            <th scope="row" className="bg-[#f0f1e9] p-5 align-top font-medium text-stone-900">{label}</th>
...
export function SavedGuides() {
  ...
  return <div className="mt-10">
    ...
    {ids.length ? <Comparison key={snapshot} ids={ids} onRemove={(id) => announce(removeSavedGuide(id) ?? 'Guide removed from your saved selection.')} /> : <div className="rounded-2xl border border-stone-200 bg-white px-6 py-16 text-center">
      <h2 className="text-2xl font-medium tracking-tight text-stone-900">No guides saved yet</h2>
