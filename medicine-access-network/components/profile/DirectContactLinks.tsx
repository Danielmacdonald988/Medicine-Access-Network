  ...
  return <div className="space-y-3">
    ...
    <div className="flex flex-col gap-2">
      {links.map(({ platform, label, href }) => (
        ...
        >
          <MessageCircle aria-hidden="true" className="size-4 shrink-0" />{t('Message on {service}', { service: label })}
          <ExternalLink aria-hidden="true" className="size-3.5 shrink-0" />
          <span className="sr-only">{' '}{t("(opens the app or a new tab)")}</span>
        </a>
      ))}
    </div>
    <p className="text-xs leading-relaxed text-stone-600">{t("Opens the app or a new tab. Your message goes directly to the guide in that service and is not stored in your guide’s website inbox. That service’s privacy settings apply.")}</p>
  </div>
}
